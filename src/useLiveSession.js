...
import { readyCaption } from './appUiState';
...
import { createFunctionResponseEnvelope } from './liveSessionTransport';
...
/**
 * Encapsula o ciclo de vida completo da sessao Live Gemini: criacao, transporte,
 * streaming de audio/video, execucao de tool calls e parada.
 *
 * @param {object} deps - Dependencias injetadas pelo App
 */
export function useLiveSession({
  ...
  const stopLiveSession = async () => {
    ...
    await flushAliceMemory().catch(() => {
      // Ignora erros de persistencia durante o encerramento para manter a parada responsiva.
    });
  ...
  return { liveOrchestratorRef, screenCleanupRef, startLiveSession, stopLiveSession };
}
